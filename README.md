...
XR Teleoperation: unitreerobotics/xr_teleoperate (VR teleoperation via Apple Vision Pro, Meta Quest, Pico)
Unitree LeRobot: unitreerobotics/unitree_lerobot (Unitree integration & utilities for LeRobot)
Hugging Face LeRobot: huggingface/lerobot (Core imitation learning policies and dataset structures)
---
2. Technical Pipeline Workflow
Environment Initialization: Environment setup using isolated virtual environments (Conda and `uv`).
Teleoperation & Control: Real-time teleoperation with Apple Vision Pro (AVP) or Meta Quest headsets.
Data Recording: Episode recording conducted across both simulation and real-world execution.
...
Policy Training: Training execution using Isaac-GR00T.
Inference & Benchmarking: Inference deployment tested on a fixed-base 29-DoF Unitree G1 humanoid robot.
---
